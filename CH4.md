...
: 퍼셉트론은 선형함수를 통과한 값 z를 계단함수로 보내 0보다 큰지, 작은지 검사하여 1과 -1로 분류하는 아주 간단한 알고리즘이며 계단 함수의 결과를 사용하여 가중치와 절편을 업데이트한다.
...
| ------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 가중치에 대한 미분 | <img src="/Users/mac/Library/Application Support/typora-user-images/image-20210131112001717.png" alt="image-20210131112001717" style="zoom:25%;" /> | <img src="/Users/mac/Library/Application Support/typora-user-images/image-20210131111759762.png" alt="image-20210131111759762" style="zoom:25%;" /> |
| 절편에 대한 미분   | <img src="/Users/mac/Library/Application Support/typora-user-images/image-20210131112032954.png" alt="image-20210131112032954" style="zoom:25%;" /> | <img src="/Users/mac/Library/Application Support/typora-user-images/image-20210131111816204.png" alt="image-20210131111816204" style="zoom:25%;" /> |
...
: 사실 우리는 이미 단일층 신경망을 구현하였다. 로지스틱 회귀는 단일층 신경망(single layer neural network)과 동일하기 때문이다. 하지만 지금까지는 층이라는 개념을 전혀 사용하지 않았다. 이제 신경망과 관련된 개념을 제대로 정리 해보자.

### 일반적인 신경망의 모습을 알아봅니다
: 일반적으로 신경망은 가장 왼쪽이 입력층, 가장 오른쪽이 출력층, 가운데 층들을 은닉층이라고 부른다. 각 층에 오른쪽에 작은 원으로 표시된 활성화 함수는 은닉층과 출력층의 한 부분으로 간주한다

<img src="https://miro.medium.com/max/440/0*Cq09Qw6Rhouay8Bg.png" alt="신경망 이미지 검색결과" style="zoom:50%;" />
...
- 예를 들어 선형회귀나 로지스틱 회귀는 모두 경사 하강법을 사용

  : 경사 하강법은 손실 함수(제곱 오차 손실 함수, 로지스틱 손실 함수)의 결과값을 최소화하는 방향으로 가중치를 업데이트 함

[colab ch04-6 code 참고](https://colab.research.google.com/drive/1_f0usEgcf82GWCgDkSKpu3nuxODpgl87)

### 여러가지 경사 하강법에 대해 알아봅니다

1. 확률적 경사 하강법(SGD : Stochastic Gradient Descent)
   - 샘플 데이터 1개에 대한 그레디언트를 계산하여 가중치를 업데이트 함
   - 장점 : 계산비용이 적음
   - 단점: 가중치가 최적값에 수렴하는 과정이 불안정함
2. 배치 경사 하강법(Batch gradient descent)
   - 전체 훈련 세트를 사용하여 한 번에 그레디언트를 계산하는 방식
   - 장점 : 가중치가 최적값에 수렴하는 과정은 안정적임
   - 단점 : 계산비용이 많이 듬

![확률적 경사 하강법 이미지 검색결과](https://blog.kakaocdn.net/dn/b8RUdK/btqAm4Utey2/5DfjTQw70XKYgtOB8VHpeK/img.png)

**미니 배치 경사 하강법(Mini-batch gradient descent)**

- 배치 크기를 작게 하여(훈련 세트를 여러 번 나누어) 처리하는 방식
- 특징 
  1. 확률적 경사 하강법과 배치 경사 하강법의 장점을 절충한 방식
  2. 그래프가 경사하강법 보다는 덜 매끄럽고 확률적 경사 하강법 보다는 매끄럽게 나타남

<img src="/Users/mac/Library/Application Support/typora-user-images/image-20210207102546813.png" alt="image-20210207102546813" style="zoom:20%;" />
...
: 사이킷런의 경사 하강법이 구현된 클래스는 SGDClassifier이다.  이 클래스는 로지스틱 회귀 문제 외에도 여러가지 문제에 경사 하강법을 적용할 수 있다. 여기서는 이 클래스를 통해 로지스틱 회귀 문제를 간단히 해결해보도록 하자
...
1. 로지스틱 손실 함수 지정하기
   ...
   - random_state : 난수 초깃값을 42로 설정하여 반복 실행했을 때 결과를 동일하게 재현하기 위함

   - tol : 반복할 때마다 로지스틱 손실 함수의 값이 해당 변수에 지정한 값만큼 감소되지 않으면 반복을 중단하도록 지정

     - 해당 값을 설정하지 않을 경우 max_iter의 값을 늘리라는 경고가 발생
     - 이는 모델의 로지스틱 손실 함수의 값이 최적값으로 수렴할 정도로 충분한 반복 횟수를 입력했는지 사용자에게 알려줌

     

2. 사이킷런으로 훈련하고 평가하기

   : 앞에서 우리가 직접 구현한 fit(), score() 메서드가 사이킷런의 SGDClassifier에 구현되어 있음
...
3. 사이킷런으로 예측하기
   ...
   : 마찬가지로 predict() 메서드가 구현되어 있음.

   - 주의 할 점 : 사이킷런은 입력 데이터로 2차원 배열만 받아들이므로, 샘플 하나를 주입하더라도 2차원 배열로 만들어야 함
